class Middleman::CoreExtensions::Internationalization < ::Middleman::Extension
  ...
  def after_configuration
    ...
    watcher = app.files.watch :locales,
                              File.join(app.root, locales_file_path),
                              ignore: proc { |f| !(/.*(rb|yml|yaml)$/.match(f[:relative_path])) }

    # Setup data files before anything else so they are available when
    # parsing config.rb
    watcher.changed(&method(:on_file_changed))
    watcher.deleted(&method(:on_file_changed))
